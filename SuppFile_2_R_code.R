...
## Installs/loads packages if not already there, loads if they are

packages <- c("tidyr","dplyr","readxl","stringr","data.table","ggplot2","LaCroixColoR","readxl", 
              "ggpubr", "gridExtra", "RColorBrewer","colorspace")
...
## If you have issues installing LaCroixColoR, download directly via devtools as follows: 
#install.packages("devtools")
#devtools::install_github("johannesbjork/LaCroixColoR")


### CREATE A NEW DIRECTORY TO WORK OUT OF
## You do not need to re-run this step once these folders are created
path<-file.path(Sys.getenv("USERPROFILE"),"Desktop",fsep="\\" )
dir.create(paste(path,"/MOC_Ecotaxa_Analysis", sep=""))
setwd(file.path(Sys.getenv("USERPROFILE"),"Desktop\\MOC_Ecotaxa_Analysis",fsep="\\"))

## FOR MAC
#dir.create(file.path("~/Desktop/MOC_Ecotaxa_Analysis"))
#setwd(file.path("~/Desktop/MOC_Ecotaxa_Analysis"))

## This creates the folder framework in your directory
dir.create("Output")
dir.create("Output/NetRaw")
dir.create("Output/NetFilter")
dir.create("Output/PairedNets")

### LOAD DATA, EXTRACT COLUMNS NEEDED #######################
#COPY YOUR ECOTAXA EXPORT DATA SET INTO THE FOLDER CREATED ON YOUR DESKTOP; 
#MODIFY FILE NAME BELOW AS NEEDED

ecotaxa_export <- 
  read.delim("ecotaxa_export_v3_Apr21_HG.txt", header=TRUE)
sub1<-ecotaxa_export[,c("object_id","object_depth_min","object_depth_max","object_annotation_hierarchy",
                        "object_area","object_major",	"object_minor","sample_tot_vol","acq_sub_part")]
...
levels(sub1$moc)
sub2<-filter(sub1, moc == "m4") # SELECT MOCNESS NUM FROM LEVEL LIST # e.g. M8
...
# Tlab<-(c("8.11799","10.56992","13.95321","16.81963","18.37317","19.21899","21.15143","24.57")) # APPLY TOW METADATA FOR SELECTED TOW
# H<-'AE1614_M3 (July, Night)'  # INSERT CRUISE/TOW INFORMATION                 
# Tow<-'M3'                        # INSERT TOW NUMBER                 
# DayNight<-'n'                     # INSERT N FOR NIGHT, D FOR DAY

# Tlab<-(c("8.1096","10.5446","14.0613","16.7541","18.3510","19.3074","20.1076","24.7042")) # APPLY TOW METADATA FOR SELECTED TOW
# H<-'AE1614_M4 (July, Day)'  # INSERT CRUISE/TOW INFORMATION                 
# Tow<-'M4'                        # INSERT TOW NUMBER                 
# DayNight<-'d'                     # INSERT N FOR NIGHT, D FOR DAY

# Tlab<-(c("8.0145","10.8157","14.3465","17.1593","18.6638","19.1972","20.1019","24.79898")) # APPLY TOW METADATA FOR SELECTED TOW
# H<-'AE1712_M8 (July, Day)'  # INSERT CRUISE/TOW INFORMATION                 
# Tow<-'M8'                        # INSERT TOW NUMBER                 
# DayNight<-'d'                     # INSERT N FOR NIGHT, D FOR DAY

# Tlab<-(c("7.6632","12.4439","14.3145","17.2428","18.6588","19.1529","19.9380","24.2207")) # APPLY TOW METADATA FOR SELECTED TOW
# H<-'AE1712_M9 (July, Night)'  # INSERT CRUISE/TOW INFORMATION                 
# Tow<-'M9'                        # INSERT TOW NUMBER                 
# DayNight<-'n'                     # INSERT N FOR NIGHT, D FOR DAY

# Tlab<-(c("7.4169","9.9423","14.4538","17.3115","16.6889","19.3212","20.0818","24.3045")) # APPLY TOW METADATA FOR SELECTED TOW
# H<-'AE1819_M10 (July, Day)'  # INSERT CRUISE/TOW INFORMATION                 
# Tow<-'M10'                        # INSERT TOW NUMBER                 
# DayNight<-'d'                     # INSERT N FOR NIGHT, D FOR DAY

# Tlab<-(c("7.3098","10.6053","14.2498","17.0452","18.6882","19.3100","19.9853")) # APPLY TOW METADATA FOR SELECTED TOW
# H<-'AE1819_M11 (July, Night)'  # INSERT CRUISE/TOW INFORMATION                 
# Tow<-'M11'                        # INSERT TOW NUMBER                 
# DayNight<-'n'                     # INSERT N FOR NIGHT, D FOR DAY

# Tlab<-(c("7.5802","10.36023","14.64107","17.33298","18.59808","19.13214","21.302998","25.18264")) # APPLY TOW METADATA FOR SELECTED TOW
# H<-'AE1830_M12 (October, Night)'  # INSERT CRUISE/TOW INFORMATION                 
# Tow<-'M12'                        # INSERT TOW NUMBER                 
# DayNight<-'n'                     # INSERT N FOR NIGHT, D FOR DAY

# Tlab<-(c("7.5803","10.3602","14.6411","17.20508","18.5579","19.2203","21.3057","25.3553")) # APPLY TOW METADATA FOR SELECTED TOW
...
# Tlab<-(c("17","17","17","17","17","17","17","17")) # APPLY TOW METADATA FOR SELECTED TOW
...
#sub2$DW<-(0.029*(sub2$vol)) #This assumes a specific biovolume to dry weigh conversion.
  ...
  sub2$DW<-with(sub2, ifelse(Taxa %like% "Calanoida", 0.055*vol,
...
sub2$O2_umol<-with(sub2, ifelse(Taxa %like% "Copepoda", ((exp(-0.399+(0.801*log(DW))+0.069*(temp)))/22.4),
                                  ifelse(Taxa %like% "Chaetognatha", ((exp(-0.173+(0.805*log(DW))+0.068*(temp)))/22.4),
                                         ifelse(Taxa %like% "Amphipoda", ((exp(0.407+(0.743*log(DW))+0.037*(temp)))/22.4),
                                               ifelse(Taxa %like% "Euphausiacea",((exp(0.392+(0.753*log(DW))+0.046*(temp)))/22.4),                                                     
...
sub2$CO2<-with(sub2, ifelse(Taxa %like% "Copepoda", O2_umol*0.87,
                              ifelse(Taxa %like% "Chaetognatha", O2_umol*1.35,
                                   ifelse(Taxa %like% "Amphipoda", O2_umol*1.35,
                                          ifelse(Taxa %like% "Euphausiacea",O2_umol*1.35,
                                                 ifelse(Taxa %like% "Mollusca",O2_umol*0.94,
                                                        O2_umol*0.87))))))

sub2$L_D<-as.factor(ifelse(sub2$Taxa %like%"not-living","not-living","living"))
  # Classifies objects based on living or dead status
...
write.csv(sub3,file= paste("Output/NetRaw/",Tow,"_t17_Classified_FOR_R.csv", sep=""),row.names=F)
...
### a separate sheet named M# (or whatever you designate it as)
  ...
  # Tow_for_R<-sub3
  # OR
Tow_for_R <- read.csv("Output/NetRaw/M13_Classified_FOR_R.csv", header=TRUE) 
...
# OR
  ...
  #Tow_for_R <- read_excel("C:/Users/amy.maas/Dropbox (BIOS)/ZoopGroup_LAB/BIOSSCOPE_data analyses/Histogram/Output/All Tows for R.xlsx", 
  #                             sheet = "M9", guess=1000000)
  # The following lines tell R how to treat the data columns and assign labels to the tow
Tow_for_R$cruise<-as.factor(Tow_for_R$cruise)
Tow_for_R$moc<-as.factor(Tow_for_R$moc)
Tow_for_R$net<-as.factor(Tow_for_R$net)
Tow_for_R$fraction<-as.factor(Tow_for_R$fraction)
Tow_for_R$D_N<-as.factor(Tow_for_R$D_N)
Tow_for_R$Taxa<-as.factor(Tow_for_R$Taxa)
Tow_for_R$Tow_Vol<-as.numeric(Tow_for_R$Tow_Vol)
...
M<-filter(Tow_for_R,!grepl(x, Taxa))

## Create a folder name for this run (e.g. M3_delT_allTaxa)
descr<-("Output/NetFilter/M13_delT_allTaxa")
dir.create(descr)

H<-'AE1830_M13 (October, Day)'     # Fill in sample info if not saved from above                
Tow<-'M13'                    # INSERT TOW NUMBER if not saved above.
...
thewholeshabang<-function(df, na.rm=TRUE){
  ...
  write.csv(summary,file=paste(descr,"/",Tow,"_bins.csv", sep=""), row.names = F)
  ...
  write.csv(netsum,file=paste(descr,"/",Tow,"_nets.csv", sep=""),row.names=F)
...
WF<-read.csv(paste(descr,"/M13_bins.csv", sep=""))
